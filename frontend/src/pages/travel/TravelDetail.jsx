...
function TravelDetail(){
    ...
    useEffect(() => {
        Promise.all([axiosInstance.get(`/api/place/commonInfo?contentId=${contentId}`),
                           axiosInstance.get(`/api/place/detailInfo?contentId=${contentId}&contentTypeId=${contentTypeId}`),
                           axiosInstance.get(`/api/place/reviews/${contentId}?page=${page}`),
                          ])
            .then(([responseData1, responseData2, responseData3]) => {
                if(responseData1.data.code === 200 || responseData2.data.code === 200){
        ...
        <div className="App mx-[300px]">
            ...
            <div className="grid grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <TourInfo contentTypeId={contentTypeId} item={detailInfoData}></TourInfo>
                </div>
            </div>

            <div className="mt-8">
                <h2 className="text-2xl font-bold">리뷰</h2>
                <div className="text-4xl font-semibold text-yellow-500 mt-2 mb-4">{reviewStarAvg} ★</div>
            ...
            <Page totalCount={totalCount} page={page} itemsPerPage={4} handlePageChange={handlePageChange}/>
        </div>
